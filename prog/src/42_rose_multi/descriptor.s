...
.ldt:           dd  SS_LDT                                          ; *  96: LDT セグメントセレクタ
.io:            dd  0                                               ;   100: I/Oマップベースアドレス
.fp_save:       times 108 + 4   db 0                                ; FPUコンテキスト保存領域
...
.ds_kernel:     dq  0x00CF92000000FFFF                              ; CODE 4G
...
.cs_task_1:     dq  0x00CFFA000000FFFF                              ; CODE 4G 特権レベル（CPL）を3にする
.ds_task_1:     dq  0x00CFF2000000FFFF                              ; DATA 4G 特権レベル（CPL）を3にする
.cs_task_2:     dq  0x00CFFA000000FFFF                              ; CODE 4G 特権レベル（CPL）を3にする
.ds_task_2:     dq  0x00CFF2000000FFFF                              ; DATA 4G 特権レベル（CPL）を3にする
.cs_task_3:     dq  0x00CFFA000000FFFF                              ; CODE 4G 特権レベル（CPL）を3にする
.ds_task_3:     dq  0x00CFF2000000FFFF                              ; DATA 4G 特権レベル（CPL）を3にする
.ds_task_4:     dq  0x00CFF2000000FFFF                              ; DATA 4G 特権レベル（CPL）を3にする
.ds_task_5:     dq  0x00CFF2000000FFFF                              ; DATA 4G 特権レベル（CPL）を3にする
.ds_task_6:     dq  0x00CFF2000000FFFF                              ; DATA 4G 特権レベル（CPL）を3にする
.end:

CS_TASK_0       equ (.cs_task_0 - LDT) | 4                          ; タスク0用のCSセレクタ
DS_TASK_0       equ (.ds_task_0 - LDT) | 4                          ; タスク0用のDSセレクタ
CS_TASK_1       equ (.cs_task_1 - LDT) | 4 | 3                      ; タスク1用のCSセレクタ 特権レベル（RPL）を3にする
DS_TASK_1       equ (.ds_task_1 - LDT) | 4 | 3                      ; タスク1用のDSセレクタ 特権レベル（RPL）を3にする
CS_TASK_2       equ (.cs_task_2 - LDT) | 4 | 3                      ; タスク2用のCSセレクタ 特権レベル（RPL）を3にする
DS_TASK_2       equ (.ds_task_2 - LDT) | 4 | 3                      ; タスク2用のDSセレクタ 特権レベル（RPL）を3にする
CS_TASK_3       equ (.cs_task_3 - LDT) | 4 | 3                      ; タスク3用のCSセレクタ 特権レベル（RPL）を3にする
DS_TASK_3       equ (.ds_task_3 - LDT) | 4 | 3                      ; タスク3用のDSセレクタ 特権レベル（RPL）を3にする
DS_TASK_4       equ (.ds_task_4 - LDT) | 4 | 3                      ; タスク4用のDSセレクタ 特権レベル（RPL）を3にする
DS_TASK_5       equ (.ds_task_5 - LDT) | 4 | 3                      ; タスク5用のDSセレクタ 特権レベル（RPL）を3にする
DS_TASK_6       equ (.ds_task_6 - LDT) | 4 | 3                      ; タスク6用のDSセレクタ 特権レベル（RPL）を3にする
