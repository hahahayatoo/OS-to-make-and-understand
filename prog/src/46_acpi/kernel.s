...
kernel:
        ...
        mov     eax, cr0                        ; 制御レジスタCR0のPGビットをセット
        or      eax, (1<<31)                    ; CR0 |= PG
        mov     cr0, eax
...
.10L:
        ...
        ;---------------------------------------
        mov     al, [.int_key]
        cmp     al, 0x02                        ; 1が押された場合にファイル読み込みが走る
        jne     .12E
        ...
        ; ファイルの内容を表示
        ;---------------------------------------
        mov     esi, 0x7800                     ; ESI = 読み込み先アドレス（0x7800）
        mov     [esi + 32], byte 0              ; [ESI + 32] =0 表示文字数を最大32文字に制限
...
.12E:
        ...
        ; CTRL+ALT+ENDキー
        ;---------------------------------------
        mov     al, [.int_key]                  ; AL = キーコード
        cdecl   ctrl_alt_end, eax               ; EAX = ctrl_alt_end()
        cmp     eax, 0
        je      .14E

        mov     eax, 0                          ; 電断処理は1度だけおこなう
        bts     [.once], eax
        jc      .14E
        cdecl   power_off                       ; 電断処理
