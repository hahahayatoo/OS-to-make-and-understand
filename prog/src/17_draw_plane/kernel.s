...
kernel:
        ...
        ; 8ビットの横線
        ;---------------------------------------
        mov     ah, 0x07                        ; AH = 書き込みプレーンを指定（RGB）
        ...
        mov     ah, 0x04                        ; AH = 書き込みプレーンを指定（R）
        out     dx, ax                          ; ポート出力

        mov     [0x000A_0000 + 1], byte 0xFF    ; 8ドットの横線

        mov     ah, 0x02                        ; AH = 書き込みプレーンを指定（G）
        out     dx, ax                          ; ポート出力

        mov     [0x000A_0000 + 2], byte 0xFF    ; 8ドットの横線

        mov     ah, 0x01                        ; AH = 書き込みプレーンを指定（B）
        out     dx, ax                          ; ポート出力
        ...
        lea     edi, [0x000A_0000 + 80]         ; EDI = 出力開始位置のVRAMアドレス
